...
// ✅ Importamos el input corregido
import CalculatorInput from '../components/CalculatorInput';

// --- 🛡️ SALVAVIDAS: Valores por defecto si la API falla ---
const SAFE_RATES = {
  usdt: { price: 0 },
  bcv: { price: 0 },
  euro: { price: 0 }
...
export default function CalculatorView({ rates, theme }) {
  ...
  // ✅ VISUAL: Convertimos etiquetas a "USD" para el menú desplegable
  ...
  // ✅ LÓGICA DE COBRO ESPECÍFICA (Restaurada)
  const handleShareWhatsApp = (zelleRateType = 'bcv') => {
    ...
    // Lógica de Referencias por Tipo de Cuenta
    if (acc.type === 'pago_movil' || acc.type === 'transferencia') {
        ...
        // USD + Ref Bs (Elegible BCV o USDT)
        msg += `\n\n*Monto: $${formatUsd(amountUsd)}*`;
        if (includeRef) {
            const usedRate = zelleRateType === 'bcv' ? safeRates.bcv.price : safeRates.usdt.price;
            if (usedRate > 0) {
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 animate-in fade-in duration-200">
            ...
            <div className="relative bg-white dark:bg-slate-900 w-full max-w-sm rounded-[2rem] p-6 shadow-2xl border border-slate-100 dark:border-slate-800">
                ...
                <div className="space-y-4">
                    <p className="text-sm text-slate-600 dark:text-slate-400">Toca el micrófono y di la cantidad y la moneda.</p>
...
// ✅ COMPONENTE PAYMENT SUMMARY (Con lógica Zelle)
function PaymentSummary({ selectedAccount, includeRef, onToggleRef, onBack, onConfirm, rates, calc }) {
    ...
    // Calcular Bs y USD para referencias
    ...
    let labelText = '';
    if (acc.type === 'pago_movil' || acc.type === 'transferencia') { const refUsd = rates.bcv.price > 0 ? amountBs / rates.bcv.price : 0; labelText = `Incluir referencia en Dólares ($${formatUsd(refUsd)})`; } 
    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <div className="bg-slate-100 dark:bg-slate-800 rounded-3xl p-4 border border-slate-200 dark:border-slate-700 flex items-center justify-between">
                <span className="text-xs font-bold text-slate-600 dark:text-slate-300 ml-1 leading-tight max-w-[70%]">{includeRef ? labelText : "Solo enviar datos de cuenta"}</span>
            ...
            <div className="flex gap-3">
                ...
                <button onClick={() => onConfirm(zelleRate)} className="flex-1 bg-brand hover:bg-brand-dark text-slate-900 rounded-2xl font-black text-sm uppercase tracking-wider shadow-lg shadow-brand/20 transition-all active:scale-95 py-4 flex items-center justify-center gap-2">
